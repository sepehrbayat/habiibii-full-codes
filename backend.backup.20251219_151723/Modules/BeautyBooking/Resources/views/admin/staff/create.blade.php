...
@section('title', translate('messages.Add New Staff'))
            ...
            <div class="d-flex justify-content-between flex-wrap gap-3">
                <div>
                    <h1 class="page-header-title text-break">
                        ...
                        <span>{{ translate('messages.Add New Staff') }}
                    </h1>
                    <p class="text-muted">{{ translate('Salon') }}: {{ $salon->store->name ?? 'N/A' }}</p>
                </div>
            </div>
        </div>
        <!-- End Page Header -->

        <form action="{{ route('admin.beautybooking.staff.store', $salon->id) }}" method="post" enctype="multipart/form-data">
            @csrf
