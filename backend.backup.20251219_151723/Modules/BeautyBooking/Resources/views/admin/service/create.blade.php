...
@section('title', translate('messages.Add New Service'))
...
@section('content')
    <div class="content container-fluid">
            ...
            <div class="d-flex justify-content-between flex-wrap gap-3">
                <div>
                    <h1 class="page-header-title text-break">
                        ...
                        <span>{{ translate('messages.Add New Service') }}
                    </h1>
                </div>
            </div>
        </div>
        ...
        <form action="{{ route('admin.beautybooking.service.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
                    ...
                    <div class="card mt-4">
                        ...
                        <div class="card-body">
                            <div class="row g-3">
                                ...
                                <div class="col-lg-6">
                                    <div class="form-group mb-0">
                                        <label class="input-label" for="choice_salon">
                                            {{ translate('messages.Salon') }} <span class="text-danger">*</span>
                                        </label>
                                        <select name="salon_id" id="choice_salon" 
                                                class="form-control js-select2-custom @error('salon_id') is-invalid @enderror" 
                                                data-placeholder="{{ translate('messages.select_salon') }}" required>
                                            <option value="" selected disabled>{{ translate('messages.select_salon') }}</option>
                                            @foreach($salons as $salon)
                                                <option value="{{ $salon->id }}" 
                                                        {{ old('salon_id') == $salon->id ? 'selected' : '' }}>
                                                    {{ $salon->store->name ?? 'N/A' }}
                                                </option>
                                            @endforeach
                                        </select>
                                        @error('salon_id')
                                ...
                                <div class="col-lg-12">
                                    <div class="form-group mb-0">
                                        ...
                                        <textarea name="description" id="service_description" 
                                                  ...
                                                  rows="3" 
                                                  placeholder="{{ translate('messages.type_service_description') }}">{{ old('description') }}</textarea>
                                        @error('description')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
