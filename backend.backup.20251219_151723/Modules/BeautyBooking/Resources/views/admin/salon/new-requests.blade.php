...
@section('title',translate('messages.new_salon_requests'))
    ...
    <div class="content container-fluid">
        ...
        <div class="page-header">
            <h1 class="page-header-title">
                ...
                {{translate('messages.new_salon_requests')}}</h1>
                ...
                <div class="col-md-12">
                    <div class="js-nav-scroller hs-nav-scroller-horizontal mt-2">
                        ...
                        <ul class="nav nav-tabs mb-3 border-0 nav--tabs">
                            <li class="nav-item">
                                <a class="nav-link {{request('request_type') == 'pending_salon' ? 'active' : ''}}" href="{{ route('admin.beautybooking.salon.new-requests') }}?request_type=pending_salon"   aria-disabled="true">{{translate('messages.Pending_Request')}}</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link {{request('request_type') == 'denied_salon' ? 'active' : ''}}" href="{{ route('admin.beautybooking.salon.new-requests') }}?request_type=denied_salon"  aria-disabled="true">{{translate('messages.Rejected_Request')}}</a>
                            </li>
                        </ul>
                    </div>
                </div>
                    ...
                    <form action="javascript:" id="search-form" class="search-form">
                        ...
                        <div class="input-group input--group">
                            <input id="datatableSearch_" type="search" name="search" class="form-control"
                                   placeholder="{{translate('ex_:_Search_Salon_Name')}}" value="{{isset($search_by) ? $search_by : ''}}" aria-label="{{translate('messages.search')}}" required>
                ...
                <table id="columnSearchDatatable"
                    ...
                    @foreach($salons as $key=>$salon)
                        <tr>
                            ...
                            <td>
                                <div>
                                    <a href="{{route('admin.beautybooking.salon.new-requests-details', $salon->id)}}" class="table-rest-info" alt="{{translate('view salon')}}">
                            ...
                            <td>
                                <div class="table-rest-info d-block">
                                    <div class="info">
                                        <div title="{{ ($salon->store->vendor->f_name ?? '') .' '. ($salon->store->vendor->l_name ?? '') }}" class="text--title">
                            ...
                            <td>
                                {{$salon->zone?$salon->zone->name:translate('messages.zone_deleted')}}
                            </td>
                            <td>
                                <div class="table-rest-info d-block">
                                    <div class="info">
                                        <div title="{{ $salon->business_type ?? 'Salon' }}" class="text--title">
                                            {{ ucwords($salon->business_type ?? 'Salon') }}
                                        </div>
                                        <div>
                                            <span class="font-light">
                                                {{ $salon->store?->package?->package_name ?? translate('messages.no_package') }}
                                            </span>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <td>
                                <div class="btn--container justify-content-center">
                                    @if(isset($salon->store->vendor) && $salon->store->vendor->status == 0)
                                        <button type="button"
                                                class="btn action-btn btn--varify btn-outline-varify shadow-none"
                                                data-deny="approve" data-toggle="modal"
                                                data-target="#exampleModal--approve-{{$salon->id}}"><i class="tio-done"></i>
                                        </button>
                                    @endif
                                    @if (!isset($salon->store->vendor->status) || $salon->verification_status == 0)
                                        <button type="button"
                                                class="btn action-btn btn--danger btn-outline-danger shadow-none"
                                                data-deny="cancel" data-toggle="modal"
                                                data-target="#exampleModal--cancel-{{$salon->id}}"><i class="tio-clear"></i>
                                        </button>
                                    @endif
                                    <a class="btn action-btn btn--warning btn-outline-warning"
                                       href="{{route('admin.beautybooking.salon.new-requests-details', $salon->id)}}"
                        ...
                        <div class="modal fade" id="exampleModal--approve-{{$salon->id}}" tabindex="-1" aria-labelledby="exampleModalLabel--approve"
                             aria-hidden="true">
                        ...
                        <div class="modal fade" id="exampleModal--cancel-{{$salon->id}}" tabindex="-1" aria-labelledby="exampleModalLabel--cancel"
                             aria-hidden="true">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-body pt-5 p-md-5">
                                        ...
                                        <form method="get" action="{{route('admin.beautybooking.salon.approve-or-deny',[$salon['id'],0])}}">
                                            @csrf
                                            <div class="form-floating">
                                                <label for="add-your-note-{{$salon->id}}" class="font-medium input-label text--title">{{ translate('Cancellation Note') }}
                                                ...
                                                <div class="mb-30">
                                                    <textarea
                                                        ...
                                                        name="message"
                                                        id="add-your-note-{{$salon->id}}"
                                                        maxlength="60"
                                                        required
                                                    ></textarea>
                                                    <div id="char-count-{{$salon->id}}">0/60</div>
                                                </div>
...
<script>
    $(document).ready(function() {
        // Character counter for cancellation notes
        $('textarea[id^="add-your-note-"]').on('input', function() {
            const id = $(this).attr('id').replace('add-your-note-', '');
            const length = $(this).val().length;
            $('#char-count-' + id).text(length + '/60');
        });
    });
</script>
@endpush
