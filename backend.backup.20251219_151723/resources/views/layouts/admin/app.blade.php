...
@if(Request::is('backoffice/payment/configuration*') || Request::is('backoffice/sms/configuration*') || Request::is('taxvat/*') || Request::is('admin/settings*'))
@php($module_type = 'settings')
@endif

    @if($module_type == 'rental')
        @include("rental::admin.partials._sidebar_{$module_type}")
    @elseif($module_type == 'beauty' && addon_published_status('BeautyBooking') == 1)
        @include("beautybooking::admin.partials._sidebar_beautybooking")
    @else
...
<script>
    ...
    // اطمینان از بارگذاری jQuery قبل از theme.min.js
    // Wrap theme.min.js execution in jQuery ready to prevent race conditions
    // قرار دادن اجرای theme.min.js در jQuery ready برای جلوگیری از race condition
    (function() {
        // Function to safely load and execute theme.min.js
        // تابع برای بارگذاری و اجرای ایمن theme.min.js
        function loadThemeScript() {
            // Verify jQuery is available
            // تأیید در دسترس بودن jQuery
            if (typeof jQuery === 'undefined' || typeof $ === 'undefined') {
                console.error('jQuery is not available. Cannot load theme.min.js');
                return false;
            }
            
            // Create script element and append to head for synchronous-like loading
            // ایجاد عنصر script و اضافه کردن به head برای بارگذاری شبیه synchronous
            var script = document.createElement('script');
            script.src = '{{asset('public/assets/admin')}}/js/theme.min.js';
            ...
            // Append to head (not body) to ensure it loads before body scripts
            // اضافه کردن به head (نه body) برای اطمینان از بارگذاری قبل از اسکریپت‌های body
            (document.head || document.getElementsByTagName('head')[0]).appendChild(script);
            return true;
        }
        
        // Check if jQuery is already available (vendor.min.js loaded synchronously)
        // بررسی اینکه آیا jQuery از قبل در دسترس است (vendor.min.js به صورت synchronous بارگذاری شده)
        if (typeof jQuery !== 'undefined' && typeof $ !== 'undefined') {
            // jQuery is loaded, load theme.min.js immediately
            // jQuery بارگذاری شده، theme.min.js را فوراً بارگذاری کن
            loadThemeScript();
        } else {
            // Wait for script to execute (vendor.min.js should load synchronously)
            // منتظر اجرای اسکریپت بمان (vendor.min.js باید به صورت synchronous بارگذاری شود)
            // Use requestAnimationFrame for next tick check
            // استفاده از requestAnimationFrame برای بررسی در tick بعدی
            requestAnimationFrame(function() {
                if (typeof jQuery !== 'undefined' && typeof $ !== 'undefined') {
                    loadThemeScript();
                } else {
                    // Fallback: check after a short delay
                    // Fallback: بررسی پس از تأخیر کوتاه
                    setTimeout(function() {
                        if (typeof jQuery !== 'undefined' && typeof $ !== 'undefined') {
                            loadThemeScript();
                        } else {
...
<script src="{{asset('public/assets/admin/js/view-pages/common.js')}}?v={{ time() }}"></script>
...
"use strict";
    ...
    @php($hasFirebaseConfig = isset($fcm_credentials['projectId']) && !empty($fcm_credentials['projectId']) && isset($fcm_credentials['apiKey']) && !empty($fcm_credentials['apiKey']))
    // CRITICAL FIX: Declare messaging variable in outer scope BEFORE conditional blocks
    // رفع بحرانی: اعلان متغیر messaging در scope بیرونی قبل از بلوک‌های شرطی
    // This ensures the variable always exists, preventing ReferenceError when Firebase is not configured
    // این اطمینان می‌دهد که متغیر همیشه وجود دارد، جلوگیری از ReferenceError زمانی که Firebase پیکربندی نشده است
    var messaging = null;
    ...
    try {
        ...
        messaging = firebase.messaging(); // Assign to outer scope variable
    } catch (error) {
        console.error('Firebase initialization error:', error);
        messaging = null; // Ensure variable is set even on error
    }
    @else
    // Firebase not configured - messaging already declared as null above
    // Firebase پیکربندی نشده - messaging قبلاً به عنوان null اعلان شده است
    @endif

    function startFCM() {
        // Check if Firebase messaging is available
        // بررسی اینکه آیا Firebase messaging در دسترس است
        if (!messaging) {
            console.warn('Firebase messaging is not available. Please configure Firebase credentials.');
            return;
        }
    ...
    function conversationView() {
        ...
        let user_id = getUrlParameter('user');
        let url= '{{url('/')}}/backoffice/message/view/'+conversation_id+'/' + user_id;
        $.ajax({
            url: url,
            success: function(data) {
    ...
    function vendorConversationView() {
        let conversation_id = getUrlParameter('conversation');
        let user_id = getUrlParameter('user');
        let url= '{{url('/')}}/backoffice/store/message/'+conversation_id+'/' + user_id;
        $.ajax({
            url: url,
            success: function(data) {
    ...
    function dmConversationView() {
        let conversation_id = getUrlParameter('conversation');
        let user_id = getUrlParameter('user');
        let url= '{{url('/')}}/backoffice/users/rider/message/'+conversation_id+'/' + user_id;
        $.ajax({
            url: url,
            success: function(data) {
    ...
    $(document).on('click', '.check-order', function () {
        ...
        {
            location.href = '{{url('/')}}/backoffice/parcel/orders/all?module_id=' + new_module_id;
        }
        else if(new_order_type === 'trip')
        {
            location.href = '{{url('/')}}/backoffice/rental/trip?module_id=' + new_module_id;
        }
        else
        {
            location.href = '{{url('/')}}/backoffice/order/list/all?module_id=' + new_module_id;
        }
    });
