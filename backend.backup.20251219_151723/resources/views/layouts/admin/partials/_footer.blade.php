<div class="footer">
    <div class="row justify-content-sm-between justify-content-center align-items-center">
        <div class="col text-md-start">
            @php
                $businessName = optional(\App\Models\BusinessSetting::where('key', 'business_name')->first())->value ?? config('app.name');
                $footerText = optional(\App\Models\BusinessSetting::where('key', 'footer_text')->first())->value ?? '';
            @endphp
            <p class="font-size-sm mb-0">
                &copy; {{ $businessName }}. <span
                    class="d-none d-sm-inline-block">{{ $footerText }}</span>
            </p>
        </div>
        <div class="col-auto">
