                ...
                <ul class="navbar-nav align-items-center flex-row flex-grow-1 __navbar-nav">
                    ...
                    <li class="nav-item __nav-item">
                        <a href="{{ route('admin.users.dashboard')}}" id="tourb-6"
                           class="__nav-link {{ Request::is('backoffice/users*') ? 'active' : '' }}">
                    ...
                    <li class="nav-item __nav-item">
                        <a href="{{ route('admin.transactions.store.withdraw_list')}}" id="tourb-7"
                           class="__nav-link {{ Request::is('backoffice/transactions*') ? 'active' : '' }}">
                    ...
                    <li class="nav-item __nav-item">
                        <a href="{{ route('admin.business-settings.business-setup') }}" id="tourb-3"
                           class="__nav-link {{ Request::is('backoffice/business-settings*') ? 'active' : '' }}">
                            ...
                            <svg width="14" viewBox="0 0 14 14" fill="none">
                                <path d="M2.33325 5.25L6.99992 9.91667L11.6666 5.25" stroke="#612f00" stroke-width="1.5"
                    ...
                    @if (\App\CentralLogics\Helpers::module_permission_check('order'))
                        <li class="nav-item __nav-item">
                            <a href="{{ route('admin.dispatch.dashboard')}}" id="tourb-8"
                               class="__nav-link {{ Request::is('backoffice/dispatch*') ? 'active' : '' }}">
                    ...
                    <li class="nav-item max-sm-m-0  mr-lg-3">
                        <a class="btn btn-icon rounded-circle nav-msg-icon"
                           href="{{route('admin.message.list')}}">
                            ...
                            @php
                                $message = \App\Models\Conversation::whereUserType('admin')->whereHas('last_message', function($query) {
                        ...
                        <div class="hs-unfold">
                            <div>
                                @php
                                    ...
                                    $lang = \App\Models\BusinessSetting::where('key', 'system_language')->first();
                                @endphp
                                @if ($lang)
                                    <div
                            ...
                            @if(isset($modules) && ($modules->count()>0))
                                ...
                                <div class="__nav-module-body">
                                    <div class="__nav-module-items">
                                        <!-- DEBUG: Modules count: {{ $modules->count() }}, Types: @foreach($modules as $m){{ $m->module_type }}({{ $m->id }}) @endforeach -->
                                        ...
                                        @php
                                            ...
                                            if ($module->module_type == 'rental' && addon_published_status('Rental') == 1) {
                                                ...
                                                try {
                                                    // Try the route - if it doesn't exist, fall back to admin dashboard
                                                    // تلاش برای مسیر - اگر وجود نداشت، بازگشت به داشبورد ادمین
                                                    $dashboardRoute = route('admin.beautybooking.dashboard');
                                                } catch (\Exception $e) {
                                                    // Route doesn't exist, use fallback
                                                    // مسیر وجود ندارد، استفاده از جایگزین
                                                    try {
                                                        $dashboardRoute = route('beautybooking.dashboard');
                                                    } catch (\Exception $e2) {
                                                        $dashboardRoute = route('admin.dashboard');
                                                    }
                                                }
