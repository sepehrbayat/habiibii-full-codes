...
export const ModuleSelection = ({
  ...
  // Sync openModal with location prop changes
  useEffect(() => {
  ...
  useEffect(() => {
    if (setOpenModuleSelection) {
      setOpenModuleSelection(openModal);
    }
  }, [openModal, setOpenModuleSelection]);
  useEffect(() => {
    if (data?.length > 0) {
  ...
  const persistZoneIds = (moduleItem) => {
    const zoneIds = moduleItem?.zones?.map((zone) => zone.id).filter(Boolean);
    if (zoneIds?.length) {
  ...
  const handleItemOnClick = (item) => {
    ...
    if(cartList?.carts?.length > 0 && item?.module_type === "rental"){
      ...
      const targetRoute =
        item?.module_type === ModuleTypes.BEAUTY ? "/beauty" : "/home";
  ...
  const handleSingleModule = (data) => {
    ...
    const targetRoute =
      data?.module_type === ModuleTypes.BEAUTY ? "/beauty" : "/home";
    router.push(targetRoute);
  };
  const innerContent = () => {
    if (isFetched) {
                ...
                <Grid container spacing={isXSmall ? 1 : 2} width="98%">
                  ...
                  <Grid item xs={12}>
                    <CustomAlert 
                      ...
                      text={
                        isSelected 
                          ? t("Select a different module or continue with the current one.")
                          : module_bottom
                      } 
                    />
                  </Grid>
