...
class Helpers
{
    ...
    {
        $setting = BusinessSetting::where(['key' => 'schedule_order'])->first();
...
//        }
    ...
    public static function send_order_notification($order)
    {
        ...
        try {
            ...
            if((in_array($order->payment_method, ['cash_on_delivery', 'offline_payment'])
                ...
                self::send_push_notif_to_topic($data, 'admin_message', 'order_request', url('/').'/backoffice/order/list/all');
            }
            ...
            if ($order->order_type == 'delivery' && !$order->scheduled && $status == 'pending' && $order->payment_method == 'cash_on_delivery' && config('order_confirmation_model') == 'deliveryman') {
                if ($order->store->sub_self_delivery && $push_notification_status) {
                    ...
                    if($order->store && $order->store->vendor && $push_notification_status){
                        self::send_push_notif_to_device($order->store->vendor->firebase_token, $data);
                        $web_push_link = url('/').'/seller-panel/order/list/all';
    ...
    public static function get_store_id()
    {
        try {
            ...
            if (auth('vendor')->check()) {
                $stores = auth('vendor')->user()->stores ?? [];
                return !empty($stores) ? ($stores[0]->id ?? null) : null;
            }
        } catch (\Exception $e) {
            return null;
        }
    ...
    public static function get_store_data()
    {
        try {
            if (auth('vendor_employee')->check()) {
                return auth('vendor_employee')->user()->store ?? null;
            }
            if (auth('vendor')->check()) {
                $stores = auth('vendor')->user()->stores ?? [];
    ...
    public static function module_permission_check($mod_name)
    {
        ...
        if ($mod_name == 'zone' && $adminUser->zone_id) {
            return false;
        }

        // Store role in variable for safer access pattern
        // ذخیره role در متغیر برای الگوی دسترسی ایمن‌تر
        $role = $adminUser->role;
        if (!$role) {
            return false;
        }
    ...
    /**
     * Check if current store has access to a specific module type
     * بررسی اینکه آیا فروشگاه فعلی به یک نوع ماژول خاص دسترسی دارد
     *
     * @param string $moduleType The module type to check (e.g., 'beauty', 'rental', 'food')
     * @return bool
     */
    public static function has_store_module_access(string $moduleType): bool
    {
        ...
        return $store->hasModuleAccess($moduleType);
    }
    ...
    public static function get_login_url($type){
        // Pluck value indexed by key to directly access the value by key
        // دریافت value با index key برای دسترسی مستقیم به value با استفاده از key
        $data=DataSetting::whereIn('key',['store_employee_login_url','store_login_url','admin_employee_login_url','admin_login_url','customer_login_url'
        ])->pluck('value','key')->toArray();

        // Support both old and new API for backward compatibility
        // پشتیبانی از API قدیمی و جدید برای سازگاری با نسخه‌های قبلی
        
        // New API: $type is a key name (e.g., 'admin_login_url') -> returns URL value (e.g., 'login_admin')
        // API جدید: $type یک نام کلید است (مثلاً 'admin_login_url') -> مقدار URL را برمی‌گرداند (مثلاً 'login_admin')
        if (isset($data[$type])) {
            return $data[$type];
        }
        
        // Backward compatibility for old code that passed URL values (e.g., 'login_admin'):
        // سازگاری با نسخه‌های قبلی برای کد قدیمی که مقادیر URL را ارسال می‌کرد (مثلاً 'login_admin'):
        // With new pluck('value','key'), array structure is: ['admin_login_url' => 'login_admin', ...]
        // با pluck جدید ('value','key')، ساختار آرایه است: ['admin_login_url' => 'login_admin', ...]
        // Search in VALUES (URL values like 'login_admin') to find if $type matches any URL value
        // جستجو در VALUES (مقادیر URL مثل 'login_admin') برای یافتن اینکه آیا $type با هر مقدار URL مطابقت دارد
        // Use in_array() first to check if value exists, then find the key to avoid false positive when key is 0
        // استفاده از in_array() ابتدا برای بررسی وجود مقدار، سپس یافتن کلید برای جلوگیری از false positive زمانی که کلید 0 است
        if (in_array($type, $data, true)) {
            // Find the matching key for the provided URL value to preserve original behavior
            // یافتن کلید متناظر برای مقدار URL ارائه شده جهت حفظ رفتار اصلی
            $matchedKey = array_search($type, $data, true);
            return $matchedKey !== false ? $matchedKey : false;
        }
    ...
    public static function react_domain_status_check(){
        ...
        if($data && isset($data['react_domain']) && isset($data['react_license_code'])){
            if(isset($data['react_platform']) && $data['react_platform'] == 'Habiibii'){
...
//                return rtrim($awsUrl, '/') . '/' . ltrim($awsBucket . '/' . $path . '/' . $data, '/');
    ...
    public static function subscriptionConditionsCheck($store_id ,$package_id,){
        ...
        if($store->module_type == 'rental'){
            ...
            // For beauty module, count services instead of items
            // برای ماژول زیبایی، شمارش سرویس‌ها به جای آیتم‌ها
            $salon = $store->beautySalon;
            $total_food = $salon ? $salon->services()->count() : 0;
        } else{
    ...
    public static function subscription_plan_chosen($store_id ,$package_id, $payment_method  ,$discount = 0,$pending_bill =0,$reference=null ,$type=null){
        ...
        if(data_get(self::subscriptionConditionsCheck(store_id:$store->id,package_id:$package->id) , 'disable_item_count') > 0){
            ...
            $store->save();
            // Disable items/services based on module type
            // غیرفعال کردن آیتم‌ها/سرویس‌ها بر اساس نوع ماژول
            if($store->module_type == 'rental'){
                Vehicle::where('provider_id',$store->id)->oldest()->take($disable_item_count)->update([
                    'status' => 0
                ]);
            } elseif($store->module_type == 'beauty' && addon_published_status('BeautyBooking') == 1){
                // For beauty module, disable services instead of items
                // برای ماژول زیبایی، غیرفعال کردن سرویس‌ها به جای آیتم‌ها
                $salon = $store->beautySalon;
                if($salon){
                    \Modules\BeautyBooking\Entities\BeautyService::where('salon_id', $salon->id)
                        ->oldest()
                        ->take($disable_item_count)
                        ->update(['status' => 0]);
                }
            }
            else{
