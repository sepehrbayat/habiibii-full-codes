...
{
    use ReportFilter, \Illuminate\Database\Eloquent\Factories\HasFactory;
    /**
    ...
    /**
     * Get all modules this store has access to
     * دریافت تمام ماژول‌هایی که این فروشگاه به آن‌ها دسترسی دارد
     *
     * @return BelongsToMany
     */
    public function accessibleModules(): BelongsToMany
    {
        return $this->belongsToMany(Module::class, 'store_modules', 'store_id', 'module_id')
            ->withPivot('status')
            ->withTimestamps();
    }

    /**
     * Check if store has access to a specific module type
     * بررسی اینکه آیا فروشگاه به یک نوع ماژول خاص دسترسی دارد
     *
    ...
    {
        // Always allow access to primary module
        // همیشه اجازه دسترسی به ماژول اصلی
        if ($this->module_type === $moduleType) {
            return true;
        }

        // Check if module is in accessible modules with status = true
        // بررسی اینکه آیا ماژول در ماژول‌های قابل دسترسی با وضعیت true است
    ...
    /**
     * Get array of accessible module types for this store
     * دریافت آرایه انواع ماژول‌های قابل دسترسی برای این فروشگاه
     *
     * @return array
     */
    public function getAccessibleModuleTypes(): array
    {
        $types = [];
        
        // Always include primary module
        // همیشه ماژول اصلی را شامل می‌شود
        if ($this->module_type) {
            $types[] = $this->module_type;
        }

        // Add accessible modules
        // افزودن ماژول‌های قابل دسترسی
        $accessibleTypes = $this->accessibleModules()
            ->wherePivot('status', true)
            ->pluck('module_type')
            ->toArray();

        return array_unique(array_merge($types, $accessibleTypes));
    }
    ...
    public function beautySalon(): HasOne
    {
        ...
        $salonClass = 'Modules\BeautyBooking\Entities\BeautySalon';
        ...
        if (!addon_published_status('BeautyBooking')) {
            ...
            return $this->hasOne($salonClass, 'store_id', 'id')->whereRaw('1 = 0');
        }
        ...
        if (!class_exists($salonClass, true)) {
        ...
        return $this->hasOne($salonClass, 'store_id', 'id');
    }
    ...
    {
        // Check if BeautyBooking module is published before using the relationship
        // بررسی اینکه آیا ماژول BeautyBooking منتشر شده است قبل از استفاده از رابطه
        if (!addon_published_status('BeautyBooking')) {
            // Return query that will yield no results if module is not published
            // برگرداندن کوئری که هیچ نتیجه‌ای برنمی‌گرداند اگر ماژول منتشر نشده باشد
            // This prevents errors when the module is disabled
            // این از خطاها در صورت غیرفعال بودن ماژول جلوگیری می‌کند
