...
class BusinessSettingsController extends Controller
{
    ...
    public function store_index()
    {
        ...
        if($store->module_type == 'rental' ){
            ...
            // Beauty module uses default settings view
            // ماژول زیبایی از نمای تنظیمات پیش‌فرض استفاده می‌کند
    ...
    public function active_status(Request $request)
    {
        ...
        // بررسی وجود store قبل از دسترسی به ویژگی‌ها
        if (!$store) {
            return response()->json(['errors' => [['code' => 'store_not_found', 'message' => translate('messages.store_not_found')]]], 404);
        }
        
        $store->active = !$store->active;
        $store->save();
        // Determine entity name based on module type
        // تعیین نام موجودیت بر اساس نوع ماژول
        $entityName = ($store->module && $store->module->module_type == 'rental') ? translate('provider') : translate('store');
        return response()->json(['message' => $store->active?$entityName.' '.translate('messages.opened'):$entityName.' '.translate('messages.temporarily_closed')], 200);
    }
    ...
    public function notification_index()
    {
        ...
        // Check if store exists and has module before accessing properties
        // بررسی وجود store و module قبل از دسترسی به ویژگی‌ها
        if (!$store || !$store->module) {
            Toastr::error(translate('messages.store_not_found'));
            return back();
        }
        ...
        if(StoreNotificationSetting::where('store_id',Helpers::get_store_id())->count() == 0 ){
            ...
            if($module_type == 'rental' && addon_published_status('Rental')){
                Helpers::storeRentalNotificationDataSetup(Helpers::get_store_id());
            } else {
                // Default notification setup for beauty and other modules
                // راه‌اندازی اعلان پیش‌فرض برای ماژول زیبایی و سایر ماژول‌ها
        ...
        $notificationModuleType = ($module_type == 'rental' && addon_published_status('Rental')) ? 'rental' : 'all';
        $data= StoreNotificationSetting::where('store_id',Helpers::get_store_id())->where('module_type', $notificationModuleType)->get();
