    ...
    public function w_request(Request $request)
    {
        ...
        if ((string) $w->balance >=  (string)$request['amount'] && (string)$request['amount'] > .01) {
            ...
            try
            {
                ...
                $moduleType = Helpers::get_store_data()?->module?->module_type;
                
                // Handle withdraw request notifications based on module type
                // مدیریت اعلان‌های درخواست برداشت بر اساس نوع ماژول
                if($moduleType == 'rental' && addon_published_status('Rental') && config('mail.status') && Helpers::get_mail_status('rental_withdraw_request_mail_status_admin') == '1' &&   Helpers::getRentalNotificationStatusData('admin','provider_withdraw_request','mail_status') ){
                    Mail::to($admin['email'])->send(new ProviderWithdrawRequestMail('pending',$wallet_transaction));
                } elseif($moduleType !== 'rental' && config('mail.status') && Helpers::get_mail_status('withdraw_request_mail_status_admin') == '1' &&   Helpers::getNotificationStatusData('admin','withdraw_request','mail_status')) {
