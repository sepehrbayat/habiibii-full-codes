...
class DashboardController extends Controller
{
    ...
    {
        set_time_limit(300); // Increase execution time for dashboard loading
        $params = [
            'zone_id' => $request['zone_id'] ?? 'all',
            'module_id' => Config::get('module.current_module_id'),
            'statistics_type' => $request['statistics_type'] ?? 'overall',
        ...
        if($module_type == 'beauty' && addon_published_status('BeautyBooking') == 1){
            return redirect()->route('admin.beautybooking.dashboard');
        }
        if($module_type == 'beauty' && addon_published_status('BeautyBooking') == 0){
            return view('errors.404');
        }
    ...
    public function order(Request $request)
    {
        ...
        if (!isset($params['statistics_type'])) {
            $params['statistics_type'] = $request['statistics_type'] ?? 'overall';
        }
        if (!isset($params['zone_id'])) {
            $params['zone_id'] = $request['zone_id'] ?? 'all';
        }
        session()->put('dash_params', $params);
    ...
    public function user_overview(Request $request)
    {
        ...
        if (!isset($params['user_overview'])) {
            $params['user_overview'] = $request['user_overview'] ?? 'yearly';
        }
        session()->put('dash_params', $params);
    ...
    public function commission_overview(Request $request)
    {
        ...
        if (!isset($params['commission_overview'])) {
            $params['commission_overview'] = $request['commission_overview'] ?? 'overall';
        }
        session()->put('dash_params', $params);
    ...
    {
        set_time_limit(300); // Increase execution time for heavy queries
        $params = session('dash_params');
        
        // Initialize params with default values if session data doesn't exist
        // مقداردهی اولیه params با مقادیر پیش‌فرض در صورت عدم وجود داده session
        ...
        // Ensure required keys exist with null defaults
        // اطمینان از وجود کلیدهای مورد نیاز با مقادیر پیش‌فرض null
        $params['zone_id'] = $params['zone_id'] ?? null;
        $params['module_id'] = $params['module_id'] ?? null;
        $params['statistics_type'] = $params['statistics_type'] ?? 'overall';
        $params['user_overview'] = $params['user_overview'] ?? 'overall';
        $params['commission_overview'] = $params['commission_overview'] ?? 'this_year';
        $params['business_overview'] = $params['business_overview'] ?? 'overall';
        
        // Skip expensive calculations on admin/users page for performance
        // رد کردن محاسبات سنگین در صفحه admin/users برای بهبود عملکرد
        // Check if we're on admin/users page - if so, skip expensive calculations
        // بررسی اینکه آیا در صفحه admin/users هستیم - در صورت مثبت، رد کردن محاسبات سنگین
        if ($request->is('admin/users', 'admin/users/*')) {
            // On admin/users page - skip expensive calculations
            // در صفحه admin/users - رد کردن محاسبات سنگین
            $data_os = [];
            $data_uo = [];
        } else {
            // On other pages - run calculations
            // در صفحات دیگر - اجرای محاسبات
        ...
        // Merge order stats and user overview data when not on admin/users page
        // ادغام آمار سفارشات و نمای کلی کاربران زمانی که در صفحه admin/users نیستیم
        // Use same condition as above to ensure consistency
        // استفاده از همان شرط بالا برای اطمینان از سازگاری
        if ($request->is('admin/users', 'admin/users/*')) {
            // On admin/users page - initialize empty array to skip dashboard data
            // در صفحه admin/users - مقداردهی اولیه آرایه خالی برای رد کردن داده‌های داشبورد
            $dash_data = [];
        } else {
            // On other pages - merge calculated data
            // در صفحات دیگر - ادغام داده‌های محاسبه شده
            // Safety check: ensure both arrays exist before merging
            // بررسی ایمنی: اطمینان از وجود هر دو آرایه قبل از ادغام
            $dash_data = array_merge(
                is_array($data_os) ? $data_os : [],
                is_array($data_uo) ? $data_uo : []
            );
        }
