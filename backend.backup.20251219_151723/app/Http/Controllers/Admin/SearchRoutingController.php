...
class SearchRoutingController extends Controller
{
    public function index(Request $request)
    {
        ...
        if (file_exists($jsonFilePath)) {
            ...
            // Filter out rental routes if not in rental module
            // فیلتر کردن روت‌های rental اگر در ماژول rental نیستیم
            if (!addon_published_status('Rental') || $currentModuleType !== 'rental') {
                $routes = array_filter($routes, function ($route) {
                    return  !in_array('rental', $route['moduleType']);
                });
            }
            // Filter out beauty routes if not in beauty module
            // فیلتر کردن روت‌های beauty اگر در ماژول beauty نیستیم
            if (!addon_published_status('BeautyBooking') || $currentModuleType !== 'beauty') {
                $routes = array_filter($routes, function ($route) {
                    return  !in_array('beauty', $route['moduleType']);
                });
            }
        ...
        // Exclude module-specific terms based on current module
        // رد کردن اصطلاحات خاص ماژول بر اساس ماژول فعلی
        if (!addon_published_status('Rental') || $currentModuleType !== 'rental') {
            $excludeTermsRoute[] = 'rental';
        }
        if (!addon_published_status('BeautyBooking') || $currentModuleType !== 'beauty') {
            $excludeTermsRoute[] = 'beauty';
        }
        ...
        if (is_numeric($searchKeyword) && $searchKeyword > 0) {
            ...
            if ($store) {
                ...
                } elseif($currentModuleType == 'beauty' && addon_published_status('BeautyBooking')){
                    // Beauty module uses default store routes
                    // ماژول زیبایی از روت‌های پیش‌فرض store استفاده می‌کند
                    if ($store->vendor->status === 0) {
                        $storeRoutes = $adminRoutes->filter(function ($route) {
            ...
            // Handle orders for non-rental modules (including beauty)
            // مدیریت سفارشات برای ماژول‌های غیر-rental (شامل beauty)
            if ($currentModuleType !== 'rental') {
    ...
    private function filterRoute($model, $route, $type = null, $name = null, $prefix = null, $module_type = null, $searchKeyword = null): array
    {
        ...
        if ($type === 'basic-campaign') {
            $baseURL = url('/') . '/backoffice/campaign/basic';
        ...
        if ($type === 'item-campaign') {
            $baseURL = url('/') . '/backoffice/campaign/item';
            $action = $formattedRouteName === 'Edit' ? 'edit' : 'view';
            $fullURL = "{$baseURL}/{$action}/{$model->id}";
