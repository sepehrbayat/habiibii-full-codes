...
class VendorController extends Controller
{
    ...
    public function view(Request $request,$store_id, $tab=null, $sub_tab='cash')
    {
        ...
        // Redirect to module-specific detail pages
        // هدایت به صفحات جزئیات خاص ماژول
        ...
        // Beauty module uses default vendor details view
        // ماژول زیبایی از نمای جزئیات وندور پیش‌فرض استفاده می‌کند
        ...
        else if($tab == 'reviews')
        {
            ...
            ->first();
            // Determine module type for subscription packages
            // تعیین نوع ماژول برای پکیج‌های اشتراک
            $packageModuleType = 'all';
            if($store?->module?->module_type == 'rental' && addon_published_status('Rental')){
                $packageModuleType = 'rental';
            } elseif($store?->module?->module_type == 'beauty' && addon_published_status('BeautyBooking')){
                $packageModuleType = 'beauty';
            }
            
            $packages = SubscriptionPackage::where('status',1)
            ->where('module_type', $packageModuleType)
            ->latest()->get();
    ...
    public function withdrawStatus(Request $request, $id)
    {
        ...
        // Validate store exists before proceeding
        // اعتبارسنجی وجود store قبل از ادامه
        if (!$store) {
        ...
        if ($request->approved == 1) {
            ...
            $withdraw->save();
            // Get notification status based on module type
            // دریافت وضعیت اعلان بر اساس نوع ماژول
            if($moduleType == 'rental' && addon_published_status('Rental')){
                $push_notification_status = Helpers::getRentalNotificationStatusData('provider','provider_withdraw_approve','push_notification_status',$store->id);
            } else {
                // Default notification system for beauty and other modules
                // سیستم اعلان پیش‌فرض برای ماژول زیبایی و سایر ماژول‌ها
                $push_notification_status = Helpers::getNotificationStatusData('store','store_withdraw_approve','push_notification_status',$store->id);
            }
            $push_notification_status = $push_notification_status == 1 && $vendor?->firebase_token ? 1 : 0;

            // Get mail status based on module type
            // دریافت وضعیت ایمیل بر اساس نوع ماژول
            if($moduleType == 'rental' && addon_published_status('Rental')){
                $mail_status = config('mail.status') &&  Helpers::get_mail_status('rental_withdraw_approve_mail_status_provider') == '1' &&  Helpers::getRentalNotificationStatusData('provider','provider_withdraw_approve','mail_status',$store->id);
            } else {
                // Default mail system for beauty and other modules
                // سیستم ایمیل پیش‌فرض برای ماژول زیبایی و سایر ماژول‌ها
                $mail_status = config('mail.status') &&  Helpers::get_mail_status('withdraw_approve_mail_status_store') == '1' &&  Helpers::getNotificationStatusData('store','store_withdraw_approve','mail_status',$store->id);
            }
            ...
            // Get notification status for withdrawal rejection based on module type
            // دریافت وضعیت اعلان برای رد برداشت بر اساس نوع ماژول
            if($moduleType == 'rental' && addon_published_status('Rental')){
                $push_notification_status = Helpers::getRentalNotificationStatusData('provider','provider_withdraw_rejaction','push_notification_status',$store->id);
            } else {
                // Default notification system for beauty and other modules
                // سیستم اعلان پیش‌فرض برای ماژول زیبایی و سایر ماژول‌ها
                $push_notification_status = Helpers::getNotificationStatusData('store','store_withdraw_rejaction','push_notification_status',$store->id);
            }
            $push_notification_status = $push_notification_status == 1 && $vendor?->firebase_token ? 1 : 0;

            // Get mail status for withdrawal rejection based on module type
            // دریافت وضعیت ایمیل برای رد برداشت بر اساس نوع ماژول
            if($moduleType == 'rental' && addon_published_status('Rental')){
                $mail_status = config('mail.status') &&  Helpers::get_mail_status('rental_withdraw_deny_mail_status_provider') == '1' &&  Helpers::getRentalNotificationStatusData('provider','provider_withdraw_rejaction','mail_status',$store->id);
            } else {
                // Default mail system for beauty and other modules
                // سیستم ایمیل پیش‌فرض برای ماژول زیبایی و سایر ماژول‌ها
                $mail_status = config('mail.status') &&  Helpers::get_mail_status('withdraw_deny_mail_status_store') == '1' &&  Helpers::getNotificationStatusData('store','store_withdraw_rejaction','mail_status',$store->id);
            }
        ...
        private function sentWithdrawRequestNotification($withdraw,$token,$email,$type='approved',$module_type='all' , $push_notification_status = '1', $mail_status = '1'){
            try {
                ...
                if($mail_status ==1){
                    // Send appropriate mail based on module type
                    // ارسال ایمیل مناسب بر اساس نوع ماژول
                    if($module_type == 'rental' && addon_published_status('Rental')){
                        Mail::to($email)->send(new ProviderWithdrawRequestMail($type,$withdraw));
                    } else {
                        // Default mail for beauty and other modules
                        // ایمیل پیش‌فرض برای ماژول زیبایی و سایر ماژول‌ها
                        Mail::to($email)->send(new WithdrawRequestMail($type,$withdraw));
                    }
                }
            } catch(\Exception $e) {
                info($e->getMessage());
...
//                    DB::table('stores')->upsert($chunk_store,['id','email','phone','vendor_id'],['name','logo','cover_photo','latitude','longitude','address','zone_id','module_id','minimum_order','comission','tax','delivery_time','minimum_shipping_charge','per_km_shipping_charge','maximum_shipping_charge','schedule_order','status','self_delivery_system','veg','non_veg','free_delivery','take_away','delivery','reviews_section','pos_system','active','featured']);
    ...
    /**
     * Update module access for a store
     * به‌روزرسانی دسترسی ماژول برای فروشگاه
     *
     * @param Store $store
     * @param Request $request
     * @return \Illuminate\Http\RedirectResponse
     */
    public function updateModuleAccess(Store $store, Request $request)
    {
        $request->validate([
        ...
        // Prevent disabling primary module
        // جلوگیری از غیرفعال کردن ماژول اصلی
        if ($store->module_id == $module->id && !$request->status) {
            Toastr::error(translate('messages.cannot_disable_primary_module'));
            return back();
        }

        // Check if module is active
        // بررسی اینکه آیا ماژول فعال است
        if ($module->status != 1) {
            Toastr::error(translate('messages.module_not_active'));
            return back();
        }

        // Update or create module access
        // به‌روزرسانی یا ایجاد دسترسی ماژول
        if ($request->status) {
            // Check if pivot entry exists
            // بررسی اینکه آیا ورودی pivot وجود دارد
            $pivotExists = DB::table('store_modules')
            ...
            if ($pivotExists) {
                ...
                // به‌روزرسانی pivot موجود
                DB::table('store_modules')
                    ->where('store_id', $store->id)
                    ->where('module_id', $module->id)
                    ->update([
                        'status' => true,
            ...
            // Only detach if not primary module (already checked above)
            // فقط در صورت عدم ماژول اصلی قطع می‌شود (قبلاً بررسی شده است)
            DB::table('store_modules')
                ->where('store_id', $store->id)
                ->where('module_id', $module->id)
                ->delete();
        ...
        if ($request->status) {
            Toastr::success(translate('messages.module_access_enabled'));
        } else {
            Toastr::success(translate('messages.module_access_disabled'));
        }
