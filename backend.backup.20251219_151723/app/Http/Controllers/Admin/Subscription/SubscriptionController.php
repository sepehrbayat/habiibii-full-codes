...
class SubscriptionController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Determine module parameter for redirect
        // تعیین پارامتر ماژول برای redirect
        $moduleParam = ($package->module_type == 'rental') ? 1 : 'all';
        return redirect()->route('admin.business-settings.subscriptionackage.index',[ 'module' => $moduleParam ]);
    }
    ...
    {
        // Determine module type for package filtering
        // تعیین نوع ماژول برای فیلتر کردن پکیج
        $packageModuleType = 'all';
        if($subscriptionackage->module_type == 'rental' && addon_published_status('Rental')){
            $packageModuleType = 'rental';
        } elseif($subscriptionackage->module_type == 'beauty' && addon_published_status('BeautyBooking')){
            $packageModuleType = 'beauty';
        }
        
        $packages= SubscriptionPackage::where('status',1)->where('module_type', $packageModuleType)->get();
    ...
    public function subscriberDetail($id){
        ...
        // Check if store exists before accessing properties
        // بررسی وجود store قبل از دسترسی به ویژگی‌ها
        if (!$store) {
            Toastr::error(translate('messages.store_not_found'));
            return back();
        }
        
        // Access module_type through the module relationship (loaded on line 540)
        // دسترسی به module_type از طریق رابطه module (بارگذاری شده در خط 540)
        if($store->module?->module_type === 'rental') {
            $store->loadCount('vehicles as items_count' );
        } elseif($store->module?->module_type === 'beauty' && addon_published_status('BeautyBooking') == 1) {
            // Beauty module doesn't need additional count loading
            // ماژول زیبایی نیاز به بارگذاری تعداد اضافی ندارد
        }
        ...
        if ($moduleType === 'rental' && addon_published_status('Rental')) {
            $packageModuleType = 'rental';
        } elseif ($moduleType === 'beauty' && addon_published_status('BeautyBooking')) {
            $packageModuleType = 'beauty';
        }

        $packages = SubscriptionPackage::where('status',1)->where('module_type', $packageModuleType)->latest()->get();
        $admin_commission=BusinessSetting::where('key', 'admin_commission')->first()?->value ;
        $business_name=BusinessSetting::where('key', 'business_name')->first()?->value ;
        try {
            // Fix: array_search() returns false when not found and 0 when found at index 0
            // رفع: array_search() در صورت عدم یافتن false و در صورت یافتن در ایندکس 0 مقدار 0 برمی‌گرداند
            // We need to check for false first to distinguish between "not found" and "found at index 0"
            // باید ابتدا false را بررسی کنیم تا بین "یافت نشده" و "یافت شده در ایندکس 0" تمایز قائل شویم
            if (($store->store_business_model ?? 'none') == 'commission') {
                $index = 0;
            } else {
                $packageId = $store?->store_sub_update_application?->package_id ?? null;
                $packageIds = array_column($packages->toArray(), 'id');
                $searchResult = $packageId ? array_search($packageId, $packageIds, true) : false;
                // Default to first package when not found; keep array index aligned to collection index
                // پیش‌فرض به اولین پکیج در صورت عدم یافتن؛ هم‌تراز با ایندکس کلکشن
                $index = ($searchResult !== false) ? $searchResult : 0;
            }
        } catch (\Throwable $th) {
            $index = 0;
        }
