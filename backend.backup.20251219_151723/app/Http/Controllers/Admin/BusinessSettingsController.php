...
// dd( $request['commission_business_model']);
        ...
        if ($request->type == 'user_app') {
            ...
            if ($activationMode && $activationMode->value == 1 && $driveMondBaseUrl && $driveMondBaseUrl->value != null) {
                ...
                $response = Http::post($driveMondBaseUrl->value . '/api/store-configurations', [
                    'mart_business_name' => $name->value ?? "Habiibii",
    ...
    {
        $delete_account_policy = DataSetting::withoutGlobalScope('translate')->where('type', 'admin_landing_page')->where('key', 'delete_account_policy')->first();
        $delete_account_policy_status = DataSetting::where('type', 'admin_landing_page')->where('key', 'delete_account_policy_status')->first();
        return view('admin-views.business-settings.delete_account_policy', compact('delete_account_policy', 'delete_account_policy_status'));
    }

    public function delete_account_update(Request $request)
    {
        $this->update_data($request, 'delete_account_policy');
        Toastr::success(translate('messages.delete_account_policy_updated'));
        return back();
    }

    public function delete_account_policy_status($status)
    {
        $this->policy_status_update('delete_account_policy_status', $status);
        return response()->json(['status' => "changed"]);
    }
    ...
    {
        // Check module status before allowing access
        // بررسی وضعیت ماژول قبل از اجازه دسترسی
        abort_if($request?->module_type == 'rental' && !addon_published_status('Rental'), 404);
        abort_if($request?->module_type == 'beauty' && !addon_published_status('BeautyBooking'), 404);
        
        // Return appropriate view based on module type
        // برگرداندن نمای مناسب بر اساس نوع ماژول
        if($request->module_type == 'rental' && addon_published_status('Rental')){
            return view('admin-views.business-settings.fcm-index-rental');
        } else {
            // Default FCM index for beauty and other modules
            // FCM index پیش‌فرض برای ماژول زیبایی و سایر ماژول‌ها
...
//        file_put_contents(base_path('.env'), implode(PHP_EOL, $lines) . PHP_EOL);
    ...
    public function notification_setup(Request $request)
    {
        ...
        abort_if(!addon_published_status('BeautyBooking') && $request?->module == 'beauty',404 );

        if (NotificationSetting::count() == 0) {
            Helpers::notificationDataSetup();
        }
        // Setup notification data based on module type
        // راه‌اندازی داده‌های اعلان بر اساس نوع ماژول
        if(addon_published_status('Rental') && $request?->module == 'rental'){
            Helpers::getRentalAdminNotificationSetupDatasetup();
        }
        // Beauty module uses default notification setup
        // ماژول زیبایی از راه‌اندازی اعلان پیش‌فرض استفاده می‌کند

        Helpers::addNewAdminNotificationSetupDataSetup();

        // Determine module type for notification query
        // تعیین نوع ماژول برای کوئری اعلان
        if ($request?->module == 'rental' && addon_published_status('Rental')) {
            $notificationModuleType = 'rental';
        } elseif ($request?->module == 'beauty' && addon_published_status('BeautyBooking')) {
            $notificationModuleType = 'beauty';
        } else {
            $notificationModuleType = 'all';
        }
        $data = NotificationSetting::where('module_type', $notificationModuleType)
        ...
        // برگرداندن نمای مناسب بر اساس نوع ماژول
        if($request?->module == 'rental' && addon_published_status('Rental')){
            $view = 'admin-views.business-settings.notification_setup_rental';
        } else {
            // Default notification setup view for beauty and other modules
            // نمای راه‌اندازی اعلان پیش‌فرض برای ماژول زیبایی و سایر ماژول‌ها
            $view = 'admin-views.business-settings.notification_setup';
        }
