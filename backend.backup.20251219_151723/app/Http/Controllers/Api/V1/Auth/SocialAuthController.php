    ...
    public function social_register(Request $request)
    {
        ...
        try {
            if ($request['medium'] == 'google') {
                ...
                // بررسی کاربر بر اساس social_id یا ایمیل
                $user = User::where('social_id', $social_id)->first();
                if (!$user && $email) {
                    $user = User::where('email', $email)->first();
                }

                // ثبت نام در صورت نبود کاربر
                if (!$user) {
                    $fast_name = $email ? explode('@', $email)[0] : 'AppleUser';
                    $last_name = '';
        ...
        // 🔹 ادامه کد گوگل و فیسبوک بدون تغییر
        if ($request['medium'] != 'apple' && strcmp($email, $data['email']) === 0) {
            $name = explode(' ', $data['name']);
            if (count($name) > 1) {
                $fast_name = implode(" ", array_slice($name, 0, -1));
    ...
    public function social_login(Request $request)
    {
        ...
        try {
            if ($request['medium'] == 'google') {
                ...
                // فقط بخش اپل تغییر کرده
                $identityToken = $token;

                // استخراج payload از JWT
                $claims = explode('.', $identityToken)[1] ?? null;
                if (!$claims) {
                    return response()->json(['error' => 'Invalid Apple token'], 403);
                }

                $data = json_decode(base64_decode(strtr($claims, '-_', '+/')), true);

                // ایمیل و نام
                $data['email'] = $data['email'] ?? 'guest_' . uniqid() . '@apple.com';
                $data['name'] = $data['email'] ? explode('@', $data['email'])[0] : 'AppleUser';
                $appleSub = $data['sub'] ?? $unique_id;
                ...
                // پیدا کردن کاربر یا ایجاد کاربر جدید
                $user = User::where('social_id', $appleSub)->first() ?: User::where('email', $data['email'])->first();
                if (!$user) {
                    $user = new User();
                }
                ...
                $user->l_name = '';
                $user->email = $data['email'];
                $user->login_medium = 'apple';
                $user->social_id = $appleSub;
                $user->temp_token = $unique_id;
                $user->save();
            }
        } catch (\Exception $e) {
        ...
        // بررسی تطابق ایمیل برای گوگل و فیسبوک
        if (!isset($claims) && strcmp($email, $data['email']) != 0 && (!isset($data['id']) && !isset($data['kid']))) {
            return response()->json(['error' => translate('messages.email_does_not_match')], 403);
        }

        // اگر گوگل/فیسبوک و اپل باشد که در بالا کاربر ساخته شد
        if (!isset($user)) {
            $user = User::where('email', $data['email'])->first();
        }
        ...
        if (auth()->loginUsingId($user->id)) {
            ...
            // بخش OTP، ارسال پیامک و ایمیل و push notification بدون تغییر
            // ... (همه کدهای قبلی شما اینجا باقی می‌ماند)
