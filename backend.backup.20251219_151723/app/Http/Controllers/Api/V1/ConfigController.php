...
class ConfigController extends Controller
{
    ...
    public function configuration()
    {
        ...
        $currency_symbol = Cache::rememberForever("business_settings_currency_symbol", function () {
            $currency = Currency::where(['currency_code' => Helpers::currency_code()])->first();
            return $currency ? $currency->currency_symbol : '$';
        });
        $cod = json_decode($settings['cash_on_delivery'] ?? 0, true);
        $cod = is_array($cod) ? $cod : ['status' => 0];
        $digital_payment = json_decode($settings['digital_payment'] ?? 0, true);
        ...
        $social_login = [];
        foreach ((array)(Helpers::get_business_settings('social_login') ?? []) as $social) {
            $config = [
        ...
        return response()->json([
            ...
            'default_location' => ['lat' => $default_location['lat'] ?? '23.757989', 'lng' => $default_location['lng'] ?? '90.360587'],
            'currency_symbol' => $currency_symbol,
            'currency_symbol_direction' => $settings['currency_symbol_position'] ?? 'left',
            ...
            'customer_verification' => (boolean)($settings['customer_verification'] ?? 0),
            'prescription_order_status' => isset($settings['prescription_order_status']) ? (boolean)$settings['prescription_order_status'] : false,
            'schedule_order' => (boolean)($settings['schedule_order'] ?? 0),
            'order_delivery_verification' => (boolean)($settings['order_delivery_verification'] ?? 0),
            ...
            'show_dm_earning' => (boolean)($settings['show_dm_earning'] ?? 0),
            'canceled_by_deliveryman' => (boolean)($settings['canceled_by_deliveryman'] ?? 0),
            'canceled_by_store' => (boolean)($settings['canceled_by_store'] ?? 0),
            'timeformat' => (string)($settings['timeformat'] ?? '24'),
            'language' => $lang_array,
            'sys_language' => $sys_lang_array,
            'social_login' => $social_login,
            'apple_login' => $apple_login,
            'toggle_veg_non_veg' => (boolean)($settings['toggle_veg_non_veg'] ?? 0),
            'toggle_dm_registration' => (boolean)($settings['toggle_dm_registration'] ?? 0),
            'toggle_store_registration' => (boolean)($settings['toggle_store_registration'] ?? 0),
            'refund_active_status' => (boolean)($settings['refund_active_status'] ?? 0),
            ...
            'is_sms_active' =>  (boolean) (Schema::hasTable('addon_settings') && Setting::whereJsonContains('live_values->status','1')->where('settings_type', 'sms_config')->exists()),
    ...
    public function react_landing_page()
    {
        ...
        $promotional_banners = [];
        $promotionBannerRaw = $settings['promotion_banner'] ?? '[]';
        $promotionBannerDecoded = json_decode($promotionBannerRaw, true);
        if (is_array($promotionBannerDecoded)) {
            foreach ($promotionBannerDecoded as $value) {
                $img = $value['img'] ?? null;
                $storage = $value['storage'] ?? 'public';
                if ($img) {
                    $promotional_banners[] = Helpers::get_full_url('promotional_banner', $img, $storage);
                }
            }
        }
    ...
    /**
     * Get analytic scripts configuration
     * دریافت تنظیمات اسکریپت‌های آنالیتیک
     *
     * @return \Illuminate\Http\JsonResponse
     */
    public function getAnalyticScripts()
    {
        try {
            // Check if external_configurations table exists
            if (!Schema::hasTable('external_configurations')) {
                return response()->json([], 200)->header('Content-Type', 'application/json');
            }

            // Try to get analytics from external_configurations table
            // If table structure doesn't match, return empty array
            try {
                $analytics = \App\Models\ExternalConfiguration::where('type', 'analytics')
                    ->get()
                    ->map(function ($item) {
                        return [
                            'type' => $item->key ?? null,
                            'script_id' => $item->value ?? null,
                        ];
                    })
                    ->filter(function ($item) {
                        return $item['type'] && $item['script_id'];
                    })
                    ->values()
                    ->toArray();

                return response()->json($analytics, 200)->header('Content-Type', 'application/json');
            } catch (\Exception $e) {
                // If query fails (e.g., column 'type' doesn't exist), return empty array
                return response()->json([], 200)->header('Content-Type', 'application/json');
            }
        } catch (\Exception $e) {
