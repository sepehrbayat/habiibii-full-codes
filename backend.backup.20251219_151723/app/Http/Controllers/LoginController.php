...
class LoginController extends Controller
{
    ...
    {
        $this->middleware('guest:admin,vendor,customer', ['except' => 'logout']);
    }
    ...
    public function login($login_url)
    {
        ...
        $data = array_column(DataSetting::whereIn('key', ['store_employee_login_url', 'store_login_url', 'admin_employee_login_url', 'admin_login_url', 'customer_login_url'
    ...
    public function submit(Request $request)
    {
        ...
        elseif ($request->role == 'vendor') {
            ...
            // اعتبارسنجی وجود مشتری و فعال بودن آن
            ...
            // Check if customer is active (if status field exists)
            // بررسی فعال بودن مشتری (در صورت وجود فیلد status)
            if (isset($customer->status) && $customer->status == 0) {
                RateLimiter::hit($key, $decayMinutes * 60);
                return redirect()->back()->withInput($request->only('email', 'remember'))
                    ->withErrors([translate('messages.Your_account_has_been_suspended')]);
            }
        }
        ...
        if ($data == 'customer') {
            ...
            // ورود مشتری موفق - هدایت به داشبورد مشتری
            return redirect()->route('customer.dashboard')->withCookies($forgetCookies);
        }
        RateLimiter::hit($key, $decayMinutes * 60);
        return redirect()->back()->withInput($request->only('email', 'remember'))
    ...
    {
        $user_link = null; // Initialize to null for proper validation
        ...
        if (auth('vendor')?->check()) {
            ...
            // مدیریت خروج مشتری
            $user_link = Helpers::get_login_url('customer_login_url');
        ...
        // Validate user_link: check for null, false, empty string, or non-string values
        // اعتبارسنجی user_link: بررسی null، false، رشته خالی، یا مقادیر غیر رشته
        if ($user_link === null || $user_link === false || $user_link === '' || !is_string($user_link)) {
            // Fallback to home route if it exists, otherwise redirect to root
            // بازگشت به مسیر home در صورت وجود، در غیر این صورت به root
            try {
                return redirect()->route('home');
            } catch (\Exception $e) {
                // If home route doesn't exist, redirect to root URL
                // در صورت عدم وجود مسیر home، به URL اصلی هدایت می‌شود
                return redirect('/');
            }
        }
        
        // Validate that login route exists before redirecting
        // اعتبارسنجی وجود مسیر login قبل از هدایت
        try {
            return redirect()->route('login', [$user_link]);
        } catch (\Exception $e) {
            // If login route fails, fallback to home or root
            // در صورت خطا در مسیر login، بازگشت به home یا root
            try {
                return redirect()->route('home');
            } catch (\Exception $e2) {
                return redirect('/');
