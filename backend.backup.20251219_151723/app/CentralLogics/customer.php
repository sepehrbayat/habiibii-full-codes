...
class CustomerLogic
{
    ...
    public static function create_wallet_transaction($user_id, float $amount, $transaction_type, $referance)
    {
        ...
        if (in_array($transaction_type, ['add_fund_by_admin', 'add_fund', 'order_refund', 'beauty_booking_refund', 'loyalty_point', 'referrer','CashBack','subscription_refund'])) {
            $credit = $amount;
        ...
        try {
            ...
            DB::commit();
            // Return transaction object for all transaction types that should return it
            // برگرداندن شیء تراکنش برای تمام انواع تراکنش‌هایی که باید آن را برگردانند
            // This includes all credit transactions (add_fund, order_refund, beauty_booking_refund, etc.)
            // و تمام تراکنش‌های بدهی (order_place, beauty_booking, etc.) که نیاز به ردیابی دارند
            // و تراکنش‌های partial_payment که نیاز به ردیابی دارند
            if (in_array($transaction_type, [
                // Credit transactions
                'add_fund_by_admin', 'add_fund', 'order_refund', 'beauty_booking_refund', 
                'loyalty_point', 'referrer', 'CashBack', 'subscription_refund',
                // Debit transactions
                'order_place', 'trip_booking', 'beauty_booking', 'beauty_package_purchase', 
                'beauty_gift_card_purchase', 'beauty_retail_order', 'partial_payment'
            ])) {
