            ...
            switch ($class) {
                ...
                case 'App\Models\ProvideDMEarning':
                    ...
                    case 'Modules\BeautyBooking\Entities\BeautySalon':
                        // Filter by direct zone_id field (salon has its own zone_id column)
                        // فیلتر بر اساس فیلد zone_id مستقیم (سالن ستون zone_id خود را دارد)
                        $builder->where('zone_id', auth('admin')->user()->zone_id);
                        break;

                    case 'Modules\BeautyBooking\Entities\BeautyBooking':
                        // Filter by direct zone_id field (booking has its own zone_id column)
                        // فیلتر بر اساس فیلد zone_id مستقیم (رزرو ستون zone_id خود را دارد)
                        $builder->where('zone_id', auth('admin')->user()->zone_id);
                        break;

                default:
                    // No zone filtering for unknown model types
                    // عدم فیلتر منطقه برای انواع مدل ناشناخته
                    break;
            }
        }
    }
