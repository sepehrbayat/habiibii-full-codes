...
const BookingDetails = ({ bookingId }) => {
  ...
  const rescheduleTimeoutRef = useRef(null);
  ...
  const handleCancel = useCallback(() => {
    ...
    // Debounce: wait 500ms before allowing another request
  ...
  }, [bookingId, cancellationReason, isCancelling, cancelBooking, refetch]);
  ...
  const handleReschedule = useCallback(() => {
    ...
    const selectedDate = dayjs(rescheduleDate);
      ...
      rescheduleBooking(
        ...
        },
        {
          ...
          onError: (error) => {
            ...
            if (error?.response?.status === 422 || error?.response?.statusCode === 422) {
              const errs = error?.response?.data?.errors || [];
              const validationErrors = errs.map(err => err.message || err.code).filter(Boolean);
  ...
  }, [bookingId, rescheduleDate, rescheduleTime, rescheduleNotes, isWithin24Hours, isRescheduling, rescheduleBooking, refetch]);
