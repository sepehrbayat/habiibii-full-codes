...
// Default login route - redirect to customer login
// روت لاگین پیش‌فرض - هدایت به لاگین مشتری
Route::get('login', function () {
    $customerLoginUrl = \App\Models\DataSetting::where('key', 'customer_login_url')->value('value') ?? 'customer';
    return redirect()->route('login', ['tab' => $customerLoginUrl]);
})->name('login.default');
...
Route::get('logout', 'LoginController@logout')->name('logout.get'); // Keep GET for backward compatibility
...
// Test route to check Beauty module rendering
require __DIR__.'/test-beauty-module.php';

//Restaurant Registration
Route::group(['prefix' => 'seller', 'as' => 'restaurant.'], function () {
...
Route::group(['prefix' => 'rider', 'as' => 'deliveryman.'], function () {
...
Route::middleware(['auth:customer', 'web'])->group(function () {
    ...
    Route::get('/dashboard/beauty/stats', [App\Http\Controllers\Customer\DashboardController::class, 'getBeautyStats'])->name('customer.dashboard.beauty.stats');
    Route::get('/dashboard/beauty/activity', [App\Http\Controllers\Customer\DashboardController::class, 'getBeautyActivity'])->name('customer.dashboard.beauty.activity');
    Route::get('/dashboard/beauty/bookings', [App\Http\Controllers\Customer\DashboardController::class, 'getBeautyBookings'])->name('customer.dashboard.beauty.bookings');
});
