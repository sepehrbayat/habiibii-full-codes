...
const ProfileTab = ({
  ...
  const handleClickDelete = (event) => {
    // Add null check for event and currentTarget
    // افزودن بررسی null برای event و currentTarget
  ...
  const isAnchorElValid = (el) => {
    ...
    try {
      ...
      return el.isConnected !== false && document.body.contains(el);
    } catch (error) {
      return false;
    }
  };

  const open = Boolean(anchorEl) && isAnchorElValid(anchorEl);
  ...
  return (
    ...
    >
      ...
      >
        {tabMenu?.map((item, index) => {
          if (
            ...
            (!modules?.find((item) => item?.module_type === "rental") && item.id === 3) ||
            (modules?.find((item) => item?.module_type === "rental")?.status === 0 && item.id === 3) ||
            (
              [11, 12, 13, 14, 15, 16].includes(item.id) &&
      ...
      {userToken && query?.page === "profile-settings" && (
        <MoreVertIcon
          aria-describedby={id}
          variant="contained"
      ...
      {anchorEl && isAnchorElValid(anchorEl) && (
        <Popover
          open={open}
          anchorEl={anchorEl}
          onClose={handleClose}
