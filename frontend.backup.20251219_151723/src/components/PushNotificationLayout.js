...
const PushNotificationLayout = ({
  ...
  useEffect(() => {
    onMessageListener()
      ...
      .catch((err) => toast.error(safeString(err)));
    if (notification) {
        ...
        toast(
          <>
            <Stack
              ...
              width="300px"
            >
              <Typography>{safeString(notification?.title || "")}</Typography>
              <Typography>{safeString(notification?.body || "")}</Typography>
            </Stack>
          </>
        );
      }
