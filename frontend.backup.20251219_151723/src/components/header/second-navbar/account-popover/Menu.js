...
const Menu = ({ onClose, cartListRefetch }) => {
  ...
  const handleClick = (item) => {
    if (item?.id === 10) {
      ...
      // Route all menu items (including beauty items) through profile system
      router.push({
        pathname: "/profile",
        query: {
          page: item?.name,
          ...
          if (
            ...
            if (item?.id !== 8) {
              return (
                ...
                >
                  <ListItemIcon>{item?.icon && React.isValidElement(item.icon) ? item.icon : null}</ListItemIcon>
                  <ListItemText
                    sx={{
                      textTransform: "capitalize",
                    }}
                  >
                    {t((typeof item?.name === "string" ? item.name : safeString(item?.name || "")).replace("-", " "))}
                  </ListItemText>
                </MenuItem>
              );
            }
