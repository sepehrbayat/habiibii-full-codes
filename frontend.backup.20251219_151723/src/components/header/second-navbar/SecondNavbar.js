...
import { setOpenForgotPasswordModal, setSelectedModule } from "redux/slices/utils";
...
const SecondNavBar = ({ configData }) => {
  ...
  const {
    data: notificationBadgeData,
    refetch: refetchNotificationsBadge,
    isFetching: isFetchingNotificationsBadge,
  } = useQuery(
    ["beauty-notifications-badge", token, zoneId],
    async () => {
      const response = await BeautyApi.getNotifications({ limit: 20, offset: 0 });
  ...
  const {
    data: notificationPopoverData,
    refetch: refetchNotificationsPopover,
  } = useQuery(
    ["beauty-notifications-popover", token, zoneId],
    async () => {
      const response = await BeautyApi.getNotifications({ limit: 5, offset: 0 });
      return response.data;
    },
    { enabled: !!token && !!zoneId, refetchInterval: 60000 }
  );

  const handleNotificationsClick = async () => {
    if (!token) {
      router.push("/beauty/notifications");
      return;
    }
    const ids =
      notificationBadgeData?.data?.map((n) => n.id).filter(Boolean) || [];
    if (ids.length) {
      try {
        await BeautyApi.markReadNotifications(ids);
        await refetchNotificationsBadge();
      } catch (e) {
  ...
  const handleNotificationItemClick = async (notification) => {
    ...
    if (notification?.id) {
      try {
        await BeautyApi.markReadNotifications([notification.id]);
        await Promise.all([refetchNotificationsBadge(), refetchNotificationsPopover()]);
      } catch (e) {
        // ignore
      }
    }
    ...
    >
      ...
      {!isSmall && (
        ...
        >
          ...
          {token && (
            <NavBarIcon
              icon={<NotificationsNoneOutlinedIcon sx={{ fontSize: "22px" }} />}
              label={t("Notifications")}
              user="false"
              handleClick={handleNotificationsClick}
              badgeCount={
                notificationBadgeData?.unread_count
                  ? notificationBadgeData.unread_count
                  : null
              }
              onMouseEnter={(e) => setNotifAnchor(e.currentTarget)}
              onMouseLeave={() => setNotifAnchor(null)}
            />
          )}
        <Popover
          open={!!notifAnchor}
        ...
        >
          ...
          {notificationPopoverData?.data?.length ? (
            <List dense>
              {notificationPopoverData.data.map((notification) => (
                <ListItem
                  button
                  key={notification.id}
                  onClick={() => {
                ...
                >
                  <ListItemText
                    primary={notification.title || t("Notification")}
                    secondary={notification.body || notification.message || ""}
                  />
                </ListItem>
              ))}
              <ListItem
          ...
          {token ? (
            ...
            <Stack flexDirection="row">
              ...
              <Stack direction="row" justifyContent="flex-end" alignItems="end" spacing={2}>
                <SignInButton
                  onClick={() => setOpenSignIn(true)}
                  variant="contained"
                  sx={{
                    minWidth: "auto",  // عرض دکمه به اندازه محتوای آن
