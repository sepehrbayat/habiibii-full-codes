...
const WalletBoxComponent = (props) => {
  ...
  // Safely convert balance to number/string for display
  // تبدیل امن balance به number/string برای نمایش
  const getBalanceDisplay = () => {
    if (balance === null || balance === undefined) {
      return 0;
    }
    // If balance is an object, try to extract a numeric value
    // اگر balance یک object باشد، سعی کن یک مقدار عددی استخراج کنی
    if (typeof balance === "object") {
      // Try common property names
      // تلاش برای نام‌های رایج property
      ...
      // اگر هیچ property عددی پیدا نشد، 0 برگردان
      console.warn("Balance is an object but no numeric property found:", balance);
      return safeString(balance);
    }
    // If balance is already a number or string, return it
    // اگر balance از قبل number یا string است، آن را برگردان
    if (typeof balance === 'object') {
      return safeString(balance);
    }
    return balance;
            ...
            <Stack>
              ...
              >
                <Stack direction="row">
                  {!userDataLoading ? (
                    balanceDisplay !== null && balanceDisplay !== undefined
                      ? getAmountWithSign(balanceDisplay)
                      : getAmountWithSign(0)
                  ) : (
                    <Skeleton variant="text" width="100px" />
                  )}{" "}
